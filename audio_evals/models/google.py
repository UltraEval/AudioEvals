...
class Gemini(APIModel):
    ...
    def __init__(self, model_name: str = 'gemini-1.5-flash', sample_params: Dict[str, any] = None):
        super().__init__(True, sample_params)
        self.model = model_name
        assert 'GOOGLE_API_KEY' in os.environ, ValueError('not found GOOGLE_API_KEY in your ENV')
    ...
    def _inference(self, prompt: PromptStruct, **kwargs) -> str:
                ...
                for i, line in enumerate(content['contents']):
                    if line['type'] == 'text':
                        ...
                        if file_extension not in MIME_TYPE_MAP:
                            ValueError(
                                'only support [{}] type, but get {}'.format(MIME_TYPE_MAP.keys(), file_extension))

                        with open(file_path, "rb") as file:
                            file_content = file.read()
                            base64_encoded = base64.b64encode(file_content).decode('utf-8')

                        content['contents'][i] = {'inline_data': {'mime_type': MIME_TYPE_MAP[file_extension],
                                                                  'data': base64_encoded}}
            elif content['role'] == 'system':
                system_instruct = content['content']
                continue
            else:
                raise ValueError('only support role [user, system], but get {}'.format(content['role']))
        ...
        url = 'https://generativelanguage.googleapis.com/v1beta/models/{}:generateContent?key={}'.format(self.model,
                                                                                                         self.key)
