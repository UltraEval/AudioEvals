...
class AliApi(APIModel):
    ...
    def __init__(self, model_name: str = 'qwen2-audio-instruct', sample_params: Dict[str, any] = None):
        super().__init__(True, sample_params)
        self.model = model_name
        assert 'DASHSCOPE_API_KEY' in os.environ, ValueError('not found DASHSCOPE_API_KEY in your ENV')
        ...
        for content in deepcopy(prompt):
            for i, line in enumerate(content['contents']):
                if line['type'] == 'text':
                    ...
                    content['contents'][i] = {line['type']: 'file://{}'.format(line['value'])}
